...
* The `wanda` git submodule, used only to source the check JSON Schema (see <<the-check-schema>> below). Clone with submodules, or if you already cloned without them, run the following command:
+
[source,sh]
----
...
`lint` runs the validators described below and prints one line per diagnostic. `show` parses a check and pretty-prints its id, group, description, remediation and facts, useful for verifying that a check deserializes as expected.
